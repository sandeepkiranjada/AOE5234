...
if flag_J2 || flag_lunisolar
    ...
    if flag_J2
        ...
        devec_J2 = -(3*n*J2*Re^2)/(4*a^2*h^5)*( ((1-(5/h^2)*dot(zhat,h_vec)^2))*cross(h_vec,e_vec) ...
                   - (2*(dot(zhat,h_vec)))*cross(zhat,e_vec) );
        %
    ...
    if flag_lunisolar
        ...
        jd_sec = jd0 + t;
        %
        %  Sun position vector [km] (spice)
        [ ds_vec, ~ ] = cspice_spkpos( 'Sun', jd_sec, 'J2000', 'None', 'Earth' );
        ds_vec = ds_vec*1e3;         % Sun position vector [m] 
        ds = norm(ds_vec);
        ds_hat = ds_vec/ds;

        %  Moon position vector [km] (spice)
        [ mRV, ~ ] = cspice_spkezr( 'Moon', jd_sec, 'J2000', 'None', 'Earth' );
        mRV = mRV*1e3;              % Moon position vector [m] 
        dm = norm(mRV(1:3));
        dm_hat = mRV(1:3)/dm;
%         %
...
%         dm_hat = r_m'/norm(r_m);
        ...
        dHvec_sun =   (3*a^2*mu_sun)/(2*ds^3)*(5*dot(ds_hat,e_vec)*cross(e_vec,ds_hat)-dot(ds_hat,h_vec)*cross(h_vec,ds_hat));
        dHvec_moon = (3*a^2*mu_moon)/(2*dm^3)*(5*dot(dm_hat,e_vec)*cross(e_vec,dm_hat)-dot(dm_hat,h_vec)*cross(h_vec,dm_hat));
        devec_sun =   (3*mu_sun)/(2*n*ds^3)*(5*dot(ds_hat,e_vec)*cross(h_vec,ds_hat)-dot(ds_hat,h_vec)*cross(e_vec,ds_hat)-2*cross(h_vec,e_vec));
        devec_moon = (3*mu_moon)/(2*n*dm^3)*(5*dot(dm_hat,e_vec)*cross(h_vec,dm_hat)-dot(dm_hat,h_vec)*cross(e_vec,dm_hat)-2*cross(h_vec,e_vec));
        %
        %%% Compute for new dHvec, devec
